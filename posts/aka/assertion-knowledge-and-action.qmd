...
-   *X* denotes an action, agent, circumstance triple $\langle$*X~Action~, X~Agent~, X~Circumstance~*$\rangle$. We take such triples to have a truth value*.* *X* is true iff *X~Agent~* performs *X~Action~* in *X~Circumstance~*.
...
> ThingToDo(Act(*X~Agent~,*ThingToDo(*X*))) ${\leftrightarrow}$ ThingToDo(*X*)

If we substitute ThingToDo(*X*) for *p* in The Action Rule, we get this.

> Assert(*X~Agent~,*ThingToDo(*X*)) ${\rightarrow}$ ThingToDo(Act(*X~Agent~,*ThingToDo(*X*)))
