...
For any two subjects \emph{S} and S′, and any degree of belief α
necessarily, if \emph{S} and S′ have the same evidence for/against
\emph{p}, then \emph{S} is justified in believing that \emph{p} to
degree α iff S′ is, too.
\item[Threshold View]
For any two subjects \emph{S} and S′, and any degree of belief α, if
\emph{S} and S′ both believe \emph{p} to degree α, then \emph{S}
believes that \emph{p} iff S′ does too.
\item[Probabilistic Justification]
For any \emph{S}, \emph{S} is justified in believing \emph{p} iff there
is some degree of belief α such that \emph{S} is justified in believing
\emph{p} to degree α, and in S's situation, believing \emph{p} to degree
α suffices for believing \emph{p}.
\end{description}
...
following Mark Kaplan (\citeproc{ref-Kaplan1996}{1996}) in calling this
...
\emph{Bel}(\emph{p}) means that the agent believes that \emph{p}, and
A~⩾\textsubscript{\emph{q}} B means that the agent thinks A is at least
as good as B given \emph{q}, we have the following

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \emph{Bel}(\emph{p}) ↔︎ ∀A∀B∀\emph{q}(A ⩾\textsubscript{\emph{q}} B ↔︎ A
  ⩾\textsubscript{\emph{p}~∧~\emph{q}}~B)
\end{enumerate}
...
bet, B be that I decline the bet, \emph{q} be a known tautology (so my
preferences given \emph{q} are my preferences \emph{tout court}) and
...
\tightlist
\item
  \emph{Bel}(\emph{p}) ↔︎ ∀A∀B∀\emph{q} (A ⩾\textsubscript{\emph{q}} B ↔︎
  A ⩾\textsubscript{\emph{p}~∧~\emph{q}} B)
\end{enumerate}
...
\emph{inconsistently} believe both \emph{p} and ¬\emph{p}. I have some
...
\item
  ...
  satisfies ∀A∀B∀\emph{q} (A ⩾\textsubscript{\emph{q}} B ↔︎ A
  ⩾\textsubscript{\emph{p}~∧~\emph{q}} B), where the first two
  quantifiers range over the open, salient actions in the sense
  described in section 2.
\item
  For any proposition \emph{p}, and any proposition \emph{q} that is
...
\item
  ...
  belief in the proposition is greater than ½.
\item
  The previous stipulation holds both unconditionally and conditional on
  \emph{p}, for any \emph{p}.
\item
  The agent believes that \emph{p} iff ∀A∀B∀\emph{q} (A
  ⩾\textsubscript{\emph{q}} B ↔︎ A ⩾\textsubscript{\emph{p}~∧~\emph{q}}
  B), where the first two quantifiers range over all actions that are
  either open and salient \emph{tout court} (i.e.~in the sense of
  section 2) or open and salient with respect to \emph{p} (as described
  above).
\end{itemize}
...
than ½. Since my degree of belief in Caeser's foodless morning is not
greater than ½, in fact it is considerably less, I don't believe
¬\emph{p}. On the other hand, since my degree of belief in \emph{p} is
considerably greater than ½, I prefer to believe it than disbelieve it,
so I believe it.
...
and \emph{q} and their conjunction are all open or salient, and both are
believed, and the agent is probabilistically coherent, the agent also
believes \emph{p}~∧~\emph{q}. This is a quite restricted closure
principle, but this is no reason to deny that it is \emph{true}, as it
fails to be true on the threshold view.
...
\emph{q}, and \emph{p} and \emph{q} are both salient, then the agent
prefers believing \emph{p}~∧~\emph{q} to not believing it, if
\emph{p}~∧~\emph{q} is eligible for belief. In what follows
\emph{Pr}(\emph{x}~\textbar~\emph{y}) is the agent's conditional degree
of belief in \emph{x} given \emph{y}. Since the agent is coherent, we'll
assume this is a probability function (hence the name).

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Since the agent believes that \emph{q}, they prefer believing that
  \emph{q} to not believing that \emph{q} (by the criteria for belief).
\item
  So the agent prefers believing that \emph{q} to not believing that
  \emph{q} given \emph{p} (From 1 and the fact that they believe that
  \emph{p}, and that \emph{q} is salient).
\item
  So \emph{Pr}(\emph{q}~\textbar~\emph{p}) \textgreater{} ½ (from 2)
\item
  \emph{Pr}(\emph{q}~\textbar~\emph{p}) = \emph{Pr}(\emph{p}~∧~\emph{q}
  \textbar~\emph{p}) (by probability calculus)
\item
  So \emph{Pr}(\emph{p}~∧~\emph{q} \textbar~\emph{p}) \textgreater{} ½
  (from 3, 4)
\item
  So, if \emph{p}~∧~\emph{q} is eligible for belief, then the agent
  prefers believing that \emph{p}~∧~\emph{q} to not believing it, given
  \emph{p} (from 5)
\item
  So, if \emph{p}~∧~\emph{q} is eligible for belief, the agent prefers
  believing that \emph{p}~∧~\emph{q} to not believing it (from 6, and
  the fact that they believe that \emph{p}, and \emph{p}~∧~\emph{q} is
  salient)
\end{enumerate}

So whenever, \emph{p}, \emph{q} and \emph{p}~∧~\emph{q} are salient, and
the agent believes each conjunct, the agent prefers believing the
conjunction \emph{p}~∧~\emph{q} to not believing it, if
\emph{p}~∧~\emph{q} is eligible. Now we have to prove that
\emph{p}~∧~\emph{q} is eligible for belief, to prove that it is actually
...
salient \emph{tout court}.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  ∀A∀B∀\emph{r} (A ⩾\textsubscript{\emph{r}} B ↔︎ A
  ⩾\textsubscript{\emph{p}~∧~\emph{r}} B)
\item
  ∀A∀B∀\emph{r} (A ⩾\textsubscript{\emph{r}} B ↔︎ A
  ⩾\textsubscript{\emph{q}~∧~\emph{r}} B)
\item
  ∀A∀B∀\emph{r} (A ⩾\textsubscript{\emph{r}} B ↔︎ A
  ⩾\textsubscript{\emph{p}~∧~\emph{q}~∧~\emph{r}} B)
\end{enumerate}

Assume that (5) isn't true. That is, there are A, B and \emph{S} such
that ¬(A~⩾\textsubscript{\emph{s}}~B ↔︎
A~⩾\textsubscript{\emph{p}~∧~\emph{q}~∧~\emph{s}}~B). By hypothesis
\emph{S} is active, and consistent with \emph{p}~∧~\emph{q}. So it is
the conjunction of relevant, salient propositions. Since \emph{q} is
salient, this means \emph{q}~∧~\emph{s} is also active. Since \emph{S}
is consistent with \emph{p}~∧~\emph{q}, it follows that
\emph{q}~∧~\emph{s} is consistent with \emph{p}. So \emph{q}~∧~\emph{s}
is a possible substitution instance for \emph{r} in (3). Since (3) is
true, it follows that A~⩾\textsubscript{\emph{q}~∧~\emph{s}}~B ↔︎
A~⩾\textsubscript{\emph{p}~∧~\emph{q}~∧~\emph{s}}~B. By similar
reasoning, it follows that \emph{s} is a permissible substitution
instance in (4), giving us A~⩾\textsubscript{\emph{s}}~B ↔︎
A~⩾\textsubscript{\emph{q}~∧~\emph{s}}~B. Putting the last two
biconditionals together we get A~⩾\textsubscript{\emph{s}}~B ↔︎ A
⩾\textsubscript{\emph{p}~∧~\emph{q}~∧~\emph{s}}~B, contradicting our
hypothesis that there is a counterexample to (5). So whenever (3) and
(4) are true, (5) is true as well, assuming \emph{p}, \emph{q} and
\emph{p}~∧~\emph{q} are all salient.
...
isn't whether we can accept \emph{p}, accept \emph{q}, accept \emph{p}
and \emph{q} entail \emph{r}, and reject \emph{r}. As Christensen
...
real question is whether we can accept \emph{p}, accept \emph{q}, accept
\emph{p} and \emph{q} entail \emph{r}, and \emph{fail} to accept
\emph{r}. And this is always a live possibility on any threshold view,
though it seems absurd at first that this could be coherent.
...
\item
  ...
  \{\emph{q}, \emph{r}, \emph{s}\} is true.
\item
  If \emph{q} is true, A costs \$2
\item
  If \emph{r} is true, A costs \$500
\item
  If \emph{t} is true, A costs \$1
\item
  The evidence the agents has strongly supports \emph{r}, though
  \emph{t} is in fact true
\item
  Quentin believes \emph{q}
\item
  Robby believes \emph{r}
\item
...
had rational beliefs about the partition \{\emph{q}, \emph{r},
\emph{t}\} they would realise that taking the bet does not maximise
expected utility. If we take rational decisions to be those that
maximise expected utility given a rational response to the evidence,
then the decisions are clearly not rational.
...
irrationality in their belief systems concerns whether \emph{q},
\emph{r} or \emph{t} is true, not whether \emph{p} is true. If Thom
suddenly got a lot of evidence that \emph{t} is true, then all of his
(salient) beliefs would be well supported by the evidence. But it is
bizarre to think that whether his belief in \emph{p} is rational turns
on how much evidence he has for \emph{t}. Finally, even if we accept
that agents in higher stakes situations need more evidence to have
justified beliefs, the fact is that the agents are in a low-risk
situation, since \emph{t} is actually true, so the most they could lose
is \$1.
...
version of the story where they are thinking about which of \emph{q},
\emph{r} and \emph{t} are correct given their evidence when thinking
about whether to take the bet they are counterexamples to (PC).) Against
this, one might respond that if belief in \emph{p} is justified, there
are arguments one might make to the conclusion that the bet should be
taken. So it is inconsistent to say that the belief is justified, but
the decision to take the bet is not rational. The problem is finding a
premise that goes along with \emph{p} to get the conclusion that taking
the bet is rational. Let's look at some of the premises the agent might
use.
