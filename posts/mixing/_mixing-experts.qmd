...
Let *I~x~* be the disjunction of all possible evidence propositions that would lead Ava to have credence *x* in *p*. In this case *I~x~* is a simple proposition that there are 10*x* white marbles in the jar, but we don't need to assume that *I~x~* will be anything like that simple. Everything that follows about *I~x~* also holds for *Z~x~*, the disjunction of all possible evidence propositions that would lead Ava to have credence *x* in *p*, but I won't repeat the derivations. Since Quinn defers to Ava, i.e., (@expib) is true, we have the following proof. (All credences are Quinn's, so I'll drop the subscripts.) 
...
Using the general fact that *X* is equivalent to (*p* ∧ *X*) ∨ (¬*p* ∧ *X*), and that Quinn's credences are probabilistic, so his credence in an exclusive disjunction equals the sum of the credence in the disjuncts, we know this equals.
...
And that's the completely general result when the evidence the experts has is conditionally independent of both *p* and ¬*p*, and Quinn starts with credence *r* in *p*.
