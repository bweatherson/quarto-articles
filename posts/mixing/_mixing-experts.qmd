...
(@expi) If Quinn learns that Ava's credence in *p* is *x*, and nothing else, they will change their credence in *p* to *x*.

(@expz) If Quinn learns that Ben's credence in *p* is *x*, and nothing else, they will change their credence in *p* to *x*.
...
The supra-Bayesian says that this case, like every other case, ideally calls for conditionalisation. Fomrally, this means that (@expi) and (@expz) are equivalent to (@expib) and (@expzb).

(@expib) ∀*x*: *Cr~Q~*(*p* | *Cr~A~*(*p*) = *x*) = *x*
(@expzb) ∀*x*: *Cr~Q~*(*p* | *Cr~B~*(*p*) = *x*) = *x*
...
Let *A~x~* be the disjunction of all possible evidence propositions that would lead Ava to have credence *x* in *p*. In this case *A~x~* is a simple proposition that there are 10*x* white marbles in the jar, but we don't need to assume that *A~x~* will be anything like that simple. Everything that follows about *A~x~* also holds for *B~x~*, the disjunction of all possible evidence propositions that would lead Ava to have credence *x* in *p*, but I won't repeat the derivations. Since Quinn defers to Ava, i.e., (@expib) is true, we have the following proof. (All credences are Quinn's, so I'll drop the subscripts.) 

\begin{align*}
Cr(p | A_x) &= x &&\therefore \\
Cr(p ∧ A_x) &= x \cdot Cr(A_x) \\
 &= x (Cr(p ∧ A_x) + Cr(\neg p ∧ A_x)) &&\therefore \\
(1-x)Cr(p ∧ A_x) &= x \cdot Cr(\neg p ∧ A_x) &&\therefore \\
Cr(\neg p ∧ A_x) &= \frac{1-x}{x} Cr(p ∧ A_x) &&\therefore \\
...
For instance, imagine that Quinn, Ava, and Ben are part of a team trying to make a new hire. There is a particular candidate Deanna that they all think is likely the best candidate. In fact, they all think it is 0.6 likely that Deanna is the best person for the job. Ava and Ben are equally good interviewers, and they each get to separately interview Deanna. They are both impressed, and each of them independently raises their credence that Deanna is the best person to 0.8. ~
