...
citation:
  ...
  number: 3
  page: "263-282"
image: ../../images/AdobeStock_227963000.jpeg
format:
  html:
    css: ../trad_defn.css
  pdf:
    output-file: "Accuracy and the Imps"
    thanks: "Thanks to Alejandro Pérez Carballo, Richard Pettigrew, and the participants in the Arché Epistemology Seminar for helpful comments."
    include-in-header:
        text: |
          \usepackage{mathrsfs}
...
\noindent Applying Lemma 1.1 with **I** = **Brier** we get
...
\noindent Then grouping like terms and factoring out 2 yields the desired result. QED
...
\noindent Plugging the results of the last two lemmas into Lemma-1.3 produces a result of
...
\noindent Comparing this to (\#) we see that it is just $\frac{N}{4}$ times Brier$_\mathscr{S}(\mathbf{c},@)$, as we aimed to prove. QED.
...
\noindent But, since each $\pi_n \in [0, 1]$ is non-negative, it follows that $\pi_1 \geq \pi{_1}^2, \pi_2 \geq \pi{_2}^2, \dots, \pi_N \geq \pi{_N}^2$ with the inequality strict in each case unless $\pi_n$ is either 1 or 0.
...
\noindent But, by Truth Directedness $\mathbf{I}(1, 0) > \mathbf{I}(1, P(X))$ since $P(a_1) > 0$ implies that $P(X) > 0$ for all $X \in \mathscr{A}^2$. Thus $\mathbf{I}_{\mathscr{A}^2}(a_1, Q) > \mathbf{I}_{\mathscr{A}^2}(a_1, P)$.
