...
functional. The solid lines represent \(R\)-accessibility; the dotted
...
smoothly to the applications we'll be interested in.
...
\(xRy\) and \(y \geq y^\prime\) imply that \(xRy^\prime\), it follows
that whenever \(x \geq x^\prime\), then \(xR_kx^\prime\). By
refinability, if neither \(\Phi\) nor \(\neg \Phi\) are true at \(x\),
then there are successor points to \(x\) where \(\Phi\) is true, and
where \(\neg \Phi\) is true. Crucially, this means it won't be the case
that \emph{It is not true that} \(\Phi\) holds at all points \(y\) such
that \(xR_ky\). So it won't be \emph{known} that \(\Phi\) is not true.
...
general principles relating \(\geq\) and \(\Box_{ER}\), which
