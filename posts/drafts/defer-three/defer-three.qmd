...
Call the following frame **Bentham**. Again, a coin will be flipped until it lands Tails. If it ever lands Tails, F is the number of flips. If it never lands Tails, which has probability 0, then F = \infty. Again, Novice knows these facts, and so far the case is just like **Coin**. But in this case, if F = *x*, The Experimenter knows that F \leq *x*, and The Experimenter updates on that. So if F = \infty, The Experimenter learns nothing, but otherwise they can rule out all but finitely many possibilities. More precisely, P(F = *x*) = π(•|F \leq x).

The Novice probability does not Value this frame. Let Y be a random variable such that Y(F = \infty) = 0, and for all finite *n*, Y(F = *n*) = 1 - 2^-*n*^. E(Y | {*w*: Exp(Y, P(*w*)) ⩾ \twothirds}, π) = 0 < \twothirds. The only world *w* where Exp(Y, P(*w*)) ⩾ \twothirds is F = \infty, and at F = \infty, Y = 0.

On the other hand, π does Totally Trust this frame. [Note to self, I also kind of need a proof here.]
