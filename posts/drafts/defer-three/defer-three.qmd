...
The short version of the big result is that E~1~ is guaranteed to be more valuable than E~2~ iff E~1~ is more informative than E~2~. All of that needs clarifying though. Say E~1~ is a *refinement* of E~2~ iff for all *w*, E~1~(*w*) \subseteq E~2~(*w*). Formally, this is how we'll capture the intuitive notion of being more informative.^[Note that comparisons here always include equality. An experiment is a refinement of itself, is more informative than itself, and is more valuable than itself. This can lead to confusion, but it's the standard terminology, and the alternative is much more wordy.] Let O be a finite set of options: \{O~1~, \dots, O~*n*~\}. Each O~*i*~ is a function from W to reals. Intuitively, they are bets, and the 
