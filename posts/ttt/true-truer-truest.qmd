...
Consider the following (minor) variant on KT. Vary the syntax so $\square A$ is only well-formed if $A$ is of the form $B \rightarrow C$. Call the resulting logic KT~R~, with the R indicating the syntactic restriction. The restriction makes very little difference. Since $A$ is equivalent to ($A \rightarrow A$) $\rightarrow A$, even if $\square A$ is not well-formed in KT~R~, the KT-equivalent sentence $\square$(($A \rightarrow A$) $\rightarrow A$) will be well-formed. The Kripke models for KT~R~ are quite natural. $\square$($B \rightarrow C$) is true at a point iff all accessible points at which $B$ is true are points at which $C$ is true. (There is no restriction on the accessibility relation other than reflexivity.)
...
-   *R* is a subset of *W* $\times$ *W* such that *w*~1~*Rw*~2~ iff for all $A$ such that $\square A \in$ *w*~1~, $A \in$ *w*~2~
