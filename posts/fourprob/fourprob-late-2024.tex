...
\includegraphics{fourprob-late-2024_files/figure-pdf/fig-ultimatum-1.png}
...
same information set. All this is shown in
Figure~\ref{fig-newcomb}.\newpage
...
entirely forward looking. Indeed, in \textbf{?@sec-dynamic} I'll argue
against that assumption. It is to assume that if forward looking
...
permissible in Table~\ref{tbl-inverted-generic}.
...
Second, I'll assume that the values in the table settle which options
are rational. This assumption is denied by theorists such as Skyrms
...
conclusion is that given SCP, both Up and Down are permissible in these
cases.
...
All of these views violate SCP.
...
last section. The first assumption is SCP.
...
but it is very widely shared.\footnote{Indeed, it is common in
  philosophy papers to specify nothing but these values and then ask for
  intuitions about the case, which suggests many people do presuppose
  that only these values are necessary.}

The third assumption is that the labels are irrelevant. If Up is
choice-worthy in Table~\ref{tbl-generic-demon}, then Down is
choice-worthy in \textbf{?@tbl-inverted-demon}. Call this assumption
\textbf{Label Neutrality}.

The fourth assumption, which is new in this section, is that the values
in the table are utilities, and hence are only unique up to positive
affine transformation. So whatever choices are rational in a table, the
same choices are rational if each payout is multiplied by a positive
constant, and/or some (other constant) is added. Call this assumption
\textbf{Affine Transformtation}.
...
differs from \textbf{?@fig-insert-ref} in that we don't assume the coin
in stage two is fair, but we do assume the probability of Heads is known
to Chooser.)
