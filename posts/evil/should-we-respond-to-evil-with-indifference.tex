...
compelling than, say, the objections (\citeproc{ref-Keynes1921}{Keynes
1921} Ch. 4) put forward in his criticisms of earlier indifference
principles. But there still may be enough to make us reject Elga's
principle. The structure of this note is as follows. In and 2 I set out
Elga's theory, in and 4 I discuss some initial objections that I don't
think are particularly telling, in I discuss some paradoxes to which
Elga's theory seems to lead (this is reprised in where I discuss a
...
Published in \emph{Philosophy and Phenomenal Research} 70: 613-35.

Thanks to Jamie Dreier, Adam Elga and an anonymous referee for helpful
discussions about this paper and suggestions for improvements.

\end{footnotesize}}

\subsection{From Basel to Princeton}\label{from-basel-to-princeton}
...
\subsection{Add it Up}\label{add-it-up}
...
should be 1/2.\sidenote{\footnotesize INDIFFERENCE entails C-INDIFFERENCE given the
...
heavily undermined.\sidenote{\footnotesize Note also that if P-INDIFFERENCE is false,
...
\subsection{Out of sight, out of mind}\label{out-of-sight-out-of-mind}
...
experiencing. While setting out such a view, John
(\citeproc{ref-Campbell2002}{Campbell 2002, 124--26}) says that two
...
\subsection{It's Evidently
Intransitive}\label{its-evidently-intransitive}
...
\subsection{Morgan and Morgan and Morgan and
...
dissenters that are particularly relevant here. Bruno
(\citeproc{ref-deFinetti1974}{\textbf{deFinetti1974?}}) argued that
countable additivity should be rejected because it rules out the
possibility of an even distribution across the natural numbers.
...
value is a non-negative integer, and for every \emph{x}, we assign equal
probability to each hypothesis that its value is \emph{x}. Since that is
...
that?\sidenote{\footnotesize Evil's plan resembles in many respects a situation
...
\subsection{Intermission}\label{intermission}
...
that gain, but as we heard your story she wasn't.\sidenote{\footnotesize Compare the
  objection to Dutch Book arguments in Schick
...
we're sure.\sidenote{\footnotesize For example, Shafer
...
endorse.\sidenote{\footnotesize Compare the state-dependent approach to
...
some particular mistake in the composition of her credences.\sidenote{\footnotesize This
...
believe they are false, and that might be her mistake.\sidenote{\footnotesize Scylla's
...
their knowledge.\sidenote{\footnotesize This is proven in McGee
...
\subsection{Risky Business?}\label{risky-business}
...
matter of degree.\sidenote{\footnotesize Though see Hetherington
...
(\citeproc{ref-Kyburg1974}{1974}), Isaac Levi
(\citeproc{ref-Levi1974}{1974}, \citeproc{ref-Levi1980}{1980}), Richard
Jeffrey (\citeproc{ref-Jeffrey1983}{1983}) and Bas Fraassen
...
ignorance about our identity can be a sharp guide to life.\sidenote{\footnotesize Bradley
  (\citeproc{ref-monton2002}{\textbf{monton2002?}}) discusses using sets
  of probability functions to solve another problem proposed by Elga,
  the Sleeping Beauty problem (\citeproc{ref-Elga2000-ELGSBA}{Elga
  2000}). Monton notes that if Beauty's credence in \emph{The coin
  landed heads} is {[}0,~0.5{]} when she wakes up on Monday, then she
  doesn't violate van Fraassen's General Reflection Principle
...
\subsection{Boyfriend in a Coma}\label{boyfriend-in-a-coma}
...
relevant facts of the case. In that case INDIFFERENCE\sidenote{\footnotesize As with
...
perceptions \emph{entail} that he is Al and against an ``intermediate
view, according to which Al's beliefs about the setup only partially
undermine his memories of being Al. According to such a view, when Al
wakes up his credence in H ought to be slightly higher than 10\%.'' But
matters are worse than that. Elga must also reject an even weaker view
that says that Al might not know whether externalism about evidence is
true, so he does not know whether his credence in H should change. My
view is more sympathetic to that position. When Al wakes, he does not
know which direction is credences should move, or indeed whether there
is such a direction, so his credence in H should be a spread of values
including 0.1.
...
\subsection{Shooting Dice can be
...
someone.\sidenote{\footnotesize Dr Evil's plans create a situation similar to the well
...
\item
  ...
  \item
    \({\forall}\)\emph{k}
    \emph{Pr}(\emph{T}~~\emph{M}~=~2\emph{\textsuperscript{k}} - 1) =
    2\emph{\textsuperscript{k}}\textsuperscript{-1}/(2\emph{\textsuperscript{k}}~-~1)
  \item
