...
There are several assumptions here, and any one of them could be the subject of a whole paper. But they each seem very plausible, and they show how we can ground the intuition that the multi-utility approach os on the right track, and sometimes an option is not choiceworthy because it is close to the worst option on every dimension we care about.
...
2. Given Unidimensional Expectations and No Trade-Offs (as discussed in @sec-lotteries), the permissible only trade-offs are linear mixtures of the values.
