...
Published in \emph{Oxford Studies in Epistemology} 1: 311-31.
...
This paper has been presented at Cornell University and the Inland
Northwest Philosophy Conference, and each time I received valuable
feedback.
...
Thanks also to David Chalmers, Harold Hodes, Nicholas Sturgeon and,
especially, Tamar Szabó Gendler for very helpful comments on various
drafts of the paper.
...
(\citeproc{ref-Yablo2002}{2002}){]}\sidenote{\footnotesize If you prefer the
  ...
  Chalmers (\citeproc{ref-Chalmers2006}{2006}) for a thorough discussion
  of ways to interpret this phrase, and the broader notion of so-called
  `deep' contingency. Nothing that goes on here will turn on any of the
  fine distinctions made in that debate - the relevant propositions will
  be deeply contingent in every plausible sense.}. Kripke did not
...
instantiates are guaranteed to not be too badly mistaken.\sidenote{\footnotesize That
...
\item
  ...
  a priori.\sidenote{\footnotesize There is a delicate ambiguity in this expression to
...
seriously.\sidenote{\footnotesize Rationalism is supported by BonJour
...
\item
  ...
  priori.\sidenote{\footnotesize Aesthetically it would be preferable to have the
...
experiences are as if they were a normal person}.\sidenote{\footnotesize I.e. I am not
...
winter.\sidenote{\footnotesize If she is convinced that there is no possible world
...
argument.\sidenote{\footnotesize Pryor is discussing the external world sceptic, not
...
dogmatism is a version of empiricism, and it endorses (6).\sidenote{\footnotesize It
...
(11).\sidenote{\footnotesize Just which is wrong then? That depends on how ``bad'' is
...
turn out to be true.\sidenote{\footnotesize Note that there's a subtle shift here in
...
\item
  ...
  justified in believing \emph{r} a priori.\sidenote{\footnotesize \{\#fnt:ftn13
    label=``fnt:ftn13''\} David Chalmers noted that (10) and (11) entail
    that \emph{I exist} is a priori. He thought this was a bad result,
    and a sufficient reason to modify these premises. I'm perfectly
    happy with saying, following Kaplan, that \emph{I exist} is a
    priori. I don't think this proves rationalism, because I think it's
    also deeply necessary that I exist. (It's not deeply necessary that
    Brian exists, but that's no objection to what I just claimed,
    because it's not deeply necessary that I'm Brian.)
...
\item
  ...
  justified in believing \emph{r} on the basis of \emph{e}, then you are
  justified in believing \emph{r} a priori.\sidenote{\footnotesize Again, if you don't
...
world.\sidenote{\footnotesize I assume \emph{H} includes a `that's all that's relevant
...
(\citeproc{ref-Williamson2000-WILKAI}{Williamson 2000} ch.~8) has argued
...
is true in all situations within X's margin-of-error.\sidenote{\footnotesize There's a
...
(\citeproc{ref-Fara2002}{2002}) has shown, if we make certain density
assumptions on the space of available situations, we can recover the
principle (27) within this account.\sidenote{\footnotesize If we translate K as
  \(\square\) and \(\neg\)K\(\neg\) as \(\diamond\), (24) can be
  expressed as the modal formula
...
\emph{s}\textsubscript{1} and \emph{s}\textsubscript{2}.\sidenote{\footnotesize Fara
...
proposition, which I shall argue is bad.\sidenote{\footnotesize If you preferred the
...
assumed.\sidenote{\footnotesize As BonJour points out, rationalism has fallen into
