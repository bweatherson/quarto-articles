...
Website picture by
\href{https://www.flickr.com/photos/56738296@N00}{ankakay} via
\href{https://search.creativecommons.org/photos/de17378a-3148-4ae7-b101-a773da9674ef}{Creative
Commons}.
...
question\sidenote{\footnotesize Historically the impetus for allowing imprecise
...
nine axioms which must be justified in some way\sidenote{\footnotesize There is a
...
\begin{description}
\item[Independence]
For all \emph{f}, \emph{f}\(^\prime\), \emph{g}, \emph{g}\(^\prime\)
\({\in}\)~\textbf{D} and \emph{A}~\({\in}\)~\textbf{X}, if
\emph{f}~\({\equiv}\)~\emph{f}~\(^\prime\) on \emph{A},
\emph{g}~\({\equiv}\)~\emph{g}\(^\prime\) on \emph{A},
\emph{f}~\({\equiv}\)~\emph{g} on \({\lnot}\)\emph{A},
\emph{f}~\(^\prime\)~\({\equiv}\)~\emph{g}\(^\prime\) on
\({\lnot}\)\emph{A} and \emph{f}~\({\leq}\)~\emph{g} then
\emph{f}~\(^\prime\)~\({\leq}\)~\emph{g}\(^\prime\)
...
\emph{f}\(^\prime\) to \emph{g}\(^\prime\) since they know the chance of
a red or black ball being drawn but not that of a black or yellow ball.
This is easily justifiable under the Dempster-Shafer approach. Let
\emph{B}, \emph{R} and \emph{Y} be the propositions that a black, red
and yellow ball respectively is drawn. Given the evidence about the
composition of the urn, it seems plausible to set \emph{Bel}(\emph{B}) =
\emph{Bel}(\emph{R})=0,
...
value of \emph{g} is £\(\frac{1}{3}\). However the value of
\emph{f}\(^\prime\) is £\(\frac{2}{3}\) while the value of
\emph{g}\(^\prime\) is just £\(\frac{1}{3}\). Hence it is not only
acceptable, but arguably a requirement of rationality that an agent
prefer \emph{g} to \emph{f} but \emph{f}\(^\prime\) to
\emph{g}\(^\prime\).
...
\includegraphics[width=4.16667in,height=\textheight]{begging-img1.png}
...
and for \emph{f}\(^\prime\) over \emph{g} in the right-hand tree. This
is, however, insufficient for his conclusion.
...
choosing \emph{f}\(^\prime\) in the original, tabular, it seems
plausible that it is \emph{Bel}(\emph{B}\({\vee}\)\emph{R}) that
matters, not \emph{Bel}(\emph{B})+\emph{Bel}(\emph{R}). However in the
tree form problem all that matters to \emph{f}\(^\prime\) is
\emph{Bel}(\emph{B}), for the possibility that we won't need to choose,
and \emph{Bel}(\emph{R}), for the possibility that we do.

The point is that Maher has to either assume agents only consider
\emph{Bel}(\emph{B}) and \emph{Bel}(\emph{R}) when assessing
\emph{f}\(^\prime\), not \emph{Bel}(\emph{B}\({\vee}\)\emph{R}), or that
...
implausible, surely when comparing \emph{f}\(^\prime\) and
\emph{g}\(^\prime\) we just compare
\emph{Bel}(\emph{B}\({\vee}\)\emph{R}) with
...
\emph{Canadian Journal of Philosophy} 23 (sup1): 315--37.
\url{https://doi.org/10.1080/00455091.1993.10717353}.
...
\emph{Philosophy of Science} 64 (2): 291--305.
...
and Uncertainty.''} \emph{Econometrica} 9 (3/4): 298--304.
\url{https://doi.org/10.2307/1907198}.
