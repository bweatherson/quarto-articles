...
I'll end with a bit more empirical evidence to back this up. The evidence is a bit tricky because I'm claiming the existence of a cohort effect in a data set with massive age and period effects, but let me quickly step through why I believe it's there.

The data set I'm using is all the articles in those 100 journals indexed in Web of Science since 1976, and all citations between them. That's around 100,000 articles, and around 375,000 citations. @fig-facet-mean displays some important trend. Each facet describes citations to articles published in a particular year. In each facet, the x-axis is the year the publications are from, and the y-axis is the ratio between the average number of citations articles in that facet's year got, and the average number of citations that any article in the data set _which had already been published_ got.
...
article_title <- active_philo_bib |>
  as_tibble() |>
  select(id, author, art_title, journal) 
...
citation_tibble <- philo_cite |>
  ...
  rename(old_year = year)  |>
  left_join(article_years, by = c("new" = "id")) |>
  rename(new_year = year) |> # The next lines are new - restricting attention to 1976-2022
  filter(new_year <= 2022, new_year >= 1976, old_year >=1976, old_year <= 2022)
...
age_effect_tibble <- year_in_year_out |>
  filter(old_year >= 1976, old_year <= 2014, new_year >= 1979) |>
  left_join(articles_per_year, by = "old_year") |>
  select(-available) |>
  left_join(select(
...
age_effect_tibble_plot <- age_effect_tibble |>
  filter(old_year >= 1979, old_year <= 2014, new_year >= 1979) |>
...
```{r}
#| label: fig-facet-mean
#| cache: false
#| column: screen-inset
#| fig-cap: "Graph of age-effects on citation patterns."

age_effect_tibble_plot
```

Just eyeballing @fig-facet-mean we can see two things. 

First, there is an age effect which probably matches your a priori opinions. Articles aren't cited before they are published, then after publication citation rates rise very rapidly, peak, and fall off more slowly. Second, the peaks in the 1980s graphs are lower and mostly shorter than the peaks in other years. The first point is so pronounced that it makes sense to take an average of these graphs, after normalising them 
