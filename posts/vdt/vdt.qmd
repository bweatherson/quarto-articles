...
grow = right]
  ...
  \tikzstyle{level 1}=[level distance=15mm,sibling distance=17.3mm]
  \tikzstyle{level 2}=[level distance=15mm,sibling distance=17.3mm]
      
      \node[solid node,label=below:{$t_1$}]{}
        child { node [hollow node, 
                      label=below:{$t_2$}]{}
            child {
                node [square node, label=right:{$\phi$ + \$10}]{}
                edge from parent
                node[above, rotate=-30] {Accept}}
            child {
              node [square node, label=right:{$\psi$}]{}
                edge from parent
                node[above, rotate=30] {Refuse}}
            edge from parent
              node[above, rotate=-30] {Accept}
                }
          child {                 
            node [square node, label=right:{$\phi$}]{}
                edge from parent
                node[above, rotate=30] {Refuse}}
          ;
\end{tikzpicture}
