...
Published in \emph{Canadian Journal of Philosophy} 45: 692-715
...
~(\citeproc{ref-Weatherson2012}{Weatherson 2012}), but I won't be
relying on those views here.
...
left-hand side of an equality sign. \(A =_{df} B\) means that \(A\) is
...
line of his book, his view is ``knowledge first''
~(\citeproc{ref-Williamson2000}{Williamson 2000} v). And it's consistent
with `knowledge first' to say that the explanatory relationship between
evidence and rationality is complicated and multi-directional. Although
I don't endorse the knowledge first program, I agree with that last
conclusion. The explanatory relationship between evidence and
rationality is complicated and multi-directional. Evidentialism should
not be construed as denying this claim.
...
wisdom is by Maria Lasonen-Aarnio
(\citeproc{ref-Lasonen-Aarnio2010}{2010},
\citeproc{ref-Lasonen-Aarnio2014}{2014a}). And I agree with almost
...
Crispin Wright (\citeproc{ref-Wright2002}{2002},
...
Here the case is that \emph{M} forms a belief that \emph{p} at \(t_1\),
and has an apparent memory of \emph{p} at \(t_2\). As we might put it,
her memory reports that \emph{p} at this time. As above, start with the
simple case where \emph{M} knows \emph{p} at \(t_1\), and there is no
...
\item
  ...
  earlier time.\sidenote{\footnotesize As with the transmissive view on testimony, I
...
rejecting the corresponding theory of testimony.\sidenote{\footnotesize Michael
...
{\centering \includegraphics{memory-belief-and-time_files/mediabag/mbt-fig1.png}
...
{\centering \includegraphics{memory-belief-and-time_files/mediabag/mbt-fig2.png}
...
informants.\sidenote{\footnotesize On children's capacities to learn, see Saffran,
  Aslin, and Newport (\citeproc{ref-Saffran1996b}{1996};
  \citeproc{ref-Saffran1996a}{Saffran, Newport, and Aslin 1996}) and
  Gopnik et al. (\citeproc{ref-Gopnik2001}{2001}). For applications of
  this directly to the judgments of credibility, see among many others,
  Koenig, Clément, and Harris
...
\emph{S} uses her memory to (accurately) remember \(\neg D\). That is,
...
reason to think that this mnemonic belief in \(\neg D\) is sensitive;
she may well still have had it were \emph{D} true. But the direct theory
...
Fermat's Little Theorem, which says for any natural number \(n\), and
any prime \(p\), \(n^p \equiv n \text{ (mod } p)\). Using this theorem,
Tamati completes his proof, and derives a nice result \(M\).
Intuitively, Tamati has not just come to know \(M\), but he has come to
know \(M\) a priori.
...
result that when Tamati goes on to tell his friend about \(M\), the
friend gets a priori knowledge of \(M\) as well. If one thinks it is
...
\begin{description}
\item[Two Beliefs]
On Monday you came to believe that \(p\) for good reasons that justified
your belief, and on Tuesday you came to believe that \(q\) for bad
reasons that failed to justify it (where \(p\) and \(q\) are
...
\(q\). So let's assume Barnett's narration is directed at Kim. And the
question is, is Kim's belief that \(q\), on Wednesday, rational? Assume,
to make the case most interesting, that this mistaken inference to \(q\)
is completely out of character. Kim is, and knows he is, a very reliable
...
\(q\) was based is still there. Now allow the temporally local theory a
...
evidence the judgment was based on.\sidenote{\footnotesize I haven't actually defended
...
principle sychronically, so it doesn't immediately have implications for
Kim's case. But it is plausible to say that as long as the judgment
...
\(q\), and gets new evidence that would show her the old evidence was
...
\def\labelenumi{\arabic{enumi}.}
\item
  It would be irrational for Kim to re-open inquiry into whether \(q\),
...
\item
  ...
  \(q\).
\item
  Kim cannot change his attitude to \(q\) without either re-opening
  inquiry into whether \(q\), or by forgetting \(q\).
\item
  There is some rational attitude towards \(q\) that Kim can take.
\item
  So, Kim is rational to retain belief in \(q\), since any other
...
if `inquiry' is read so broadly that any re-evaluation of \(q\) counts
as re-opening inquiry. The issues are premises 1 and 4. Premise 4 is a
no dilemmas principle. We'll return to it later, though in this context
it is notable that Barnett himself endorses it, as do many other
...
avoid it.\sidenote{\footnotesize Sue Hamilton (\citeproc{ref-Hamilton2001}{2001, 78})
...
agents who make mistakes at \(t_1\) are, in virtue of those mistakes,
left without any good options at \(t_2\). Mistakes have consequences.
...
relevant fact about Kim is that he has a belief in \(q\) that is well
...
questions. In particular, \(S\)'s attitude towards \(p\) at \(t_1\) can
only be evidence for her at \(t_2\) if the following conditions obtain.
...
\item
  \(S\) does not receive a significant amount of evidence against \(p\)
  between \(t_1\) and \(t_2\).
\item
  \(S\) does not receive (undefeated) reasons to distrust her ability to
...
insignificant parts. Pazu knows that \(p\), then gets three pieces of
evidence \(e_1, e_2\) and \(e_3\) that tell against \(p\). The
conjunction is significant evidence, the individual parts are not. But
the parts come in sequentially. When \(e_1\) comes in, Pazu still knows
\(p\); after all, it is insignificant evidence. So Pazu can conclude,
i.e., know, that it is misleading evidence. And, intuitively, we can
ignore evidence we know to be misleading. So he ignores \(e_1\). And for
similar reasons he ignores \(e_2\). Then \(e_3\) comes in. Should he
...
is odd that Pazu can hold onto his knowledge in \(p\) in the face of
these three pieces of evidence, while he would have lost knowledge had
they come in at once.
...
Former Self?''} \emph{Philosophical Review} 124 (3): 353--92.
...
Philosophy} 96 (6): 271--93. \url{https://doi.org/10.2307/2564679}.
...
620--29. \url{https://doi.org/10.1037//0012-1649.37.5.620}.
...
the Royal Society B} 366: 1179--87.
\url{https://doi.org/10.1098/rstb.2010.0321}.
...
and Past Tense.''} \emph{Child Development} 79 (3): 750--60.
\url{https://doi.org/10.1111/j.1467-8624.2008.01155.x}.

\bibitem[\citeproctext]{ref-Kavka1983}
Kavka, Gregory S. 1983. {``The Toxin Puzzle.''} \emph{Analysis} 43 (1):
33--36. \url{https://doi.org/10.1093/analys/43.1.33}.
...
\emph{Psychological Science} 15 (10): 694--98.
\url{https://doi.org/10.1111/j.0956-7976.2004.00742.x}.

\bibitem[\citeproctext]{ref-Lackey2005}
Lackey, Jennifer. 2005. {``Memory as a Generative Epistemic Source.''}
\emph{Philosophy and Phenomenological Research} 70 (3): 636--58.
\url{https://doi.org/10.1111/j.1933-1592.2005.tb00418.x}.
...
Testimony?''} \emph{Philosophical Review} 115 (2): 199--241.
\url{https://doi.org/10.1215/00318108-115-2-199}.
...
(5294): 1926--28. \url{https://doi.org/10.1126/science.274.5294.1926}.
...
of Memory and Language} 35 (4): 606--21.
\url{https://doi.org/10.1006/jmla.1996.0032}.
...
https://doi.org/\url{http://dx.doi.org/10.11612/resphil.2013.90.2.8}.
...
(1): 167--212. \url{https://doi.org/10.1111/j.0309-7013.2004.00121.x}.
