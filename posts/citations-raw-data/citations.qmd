...
Around half the articles (`{r} nrow(top_five_in_main_bib)` out of `{r} article_number`) are in the journals widely taken to be the big five: Journal of Philosophy, Philosophical Review, Mind, Noûs, and Philosophy and Phenomenological Research. @tbl-recent-journals-in-main-bib shows what happens if we restrict attention to just the last fifteens years, and look at which articles from 2006 to 2020 are widely cited in this sense. The percentage that are in these five journals falls slightly: it is now `{r} nrow(recent_top_five_in_main_bib)` out of `{r} nrow(filter(main_bib, year >= 2006))`. And the order at the top has changed a bit, as @tbl-recent-journals-in-main-bib shows.
