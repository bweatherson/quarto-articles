...
year_cites |>
  ...
  select(Article = displayauth, `Citations` = all, `Citation Rank` = all_rank, `Early Citations` = early, `Early Rank` = early_rank, `Late Citations` = late, `Late Rank` =late_rank) |>
  arrange(-Citations)
}
```
...
rolling_averages <- citation_tibble |>
  ...
  left_join(main_bib, by = "old") |>
  select(old, old_year, new_year, cites, rolling, graph_cite, displayauth)
...
rolling_spaghetti_graph <- function(x){
  ggplot(filter(rolling_averages, old %in% filter(
    ...
    new_year >= x), aes(x = new_year, y = rolling, color = displayauth)) +
    geom_line() +
...
#  temp$graph_cite <- factor(temp$graph_cite, levels = temp$graph_cite, labels = temp$short_auth)
  background <- temp |> rename(backer = displayauth)
  ggplot(temp, aes(x = new_year, y = rolling, color = displayauth)) +
