...
And the last line contradicts \textbf{Possible Dependence}. So not all
six of these assumptions can be correct, if DAB is true. Since Lewis
thinks they are all correct, he concludes DAB is false.
...
arbitrary descriptive proposition A is given by \textbf{Auntie E's
Value}.
...
particular, Auntie C's values are given by \textbf{Auntie C's Value}. In
the formula, let Pr\textsubscript{A} be the result of \emph{imaging} the
credence function Pr on the proposition A. Auntie C believes changing
the moral facts is a bigger change to the world than changing any
descriptive facts, so imaging always moves credences up or down in
Table~\ref{tbl-credence}, never left or right.\footnote{Recall here that
  the worlds are epistemic possibilities, not metaphysical ones, so it
  makes sense to talk about merely changing the moral facts.}
...
This resembles equation (11) in ``Causal Decision Theory''
(\citeproc{ref-Lewis1981bn}{Lewis 1981}). Indeed, after the first
character, it just is the special case of that equation where the only
possible values are 1 and 0. But the first character matters. Lewis is
presenting a theory of usefulness, not of value. His formula is meant to
measure the thing that a rational actor maximises. It is not measuring
the thing an altruistic friend hopes is maximised. We'll come back to
this point in Section~\ref{sec-auntie-c}. For now, I just want to note
the similarities to Lewis's own theory.

Using this formula, Auntie C hopes that Peter does \emph{a} iff Pr(Å)
\textgreater{} Pr(¬Å). Since Pr(Å)~=~0.59, and Pr(¬Å)~=~0.41, that means
she does hope that Peter does \emph{a}.
...
defending the claim that this is a kind of desire as belief.
...
only rejects \textbf{Additivity} because of this confusion. But there's
