...
to reject a premiss in Lewis's argument.
\end{abstract}
...
independent reason to reject one or other of the premisses Lewis offers.
...
Lewis's own presentation, but all the premisses here are ones he uses,
and it gets to the conclusion he wants, so it seems to be a fair
...
Dependence} there is an Å such that Pr(Å) ≠ Pr(Å \textbar{} A). But
...
updating) \\
& ~=~Pr(Å~\textbar~A) & (\textbf{Restricted Conditionalisation}) \\
\end{longtable}
}

\sout{And the last line contradicts our assumption. So not all six of
these assumptions can be correct, if DAB is true. Since Lewis thinks
they are all correct, he concludes DAB is false.}

\begingroup\bfseries\boldmath

And the last line contradicts \textbf{Possible Dependence}. So the six
assumptions are jointly inconsistent. Lewis thinks the five other than
\textbf{Equation} are all secure, so he concludes that \textbf{Equation}
is false. And since \textbf{Equation} is just what DAB comes to, given
\textbf{Binary Desirability}, he concludes that DAB is false.

\endgroup

As I said, this presentation doesn't look a lot like Lewis's argument.
Most notably, \textbf{Possible Dependence} isn't an assumption for
Lewis, it is something he derives from yet further premisses. Much of
the 1988 paper is devoted to spelling out the absurd consequences of
denying \textbf{Possible Dependence}. These arguments haven't convinced
...
But \textbf{Possible Dependence} is surely true. Assume that A is a
proposition about what someone, call him Peter, might do. And assume
that we desire that the morally good thing is done, that we don't know
whether A is good or not, but we are very confident in Peter's moral
judgment. If Peter does A, that's good evidence A is good. That all
seems coherent, which is enough to support \textbf{Possible Dependence}.
...
through. {\bfseries\boldmath{}The interesting defenders of DAB are those
who reject \textbf{Equation}, and those who reject \textbf{Additivity}.}
I'm going to argue that anyone sympathetic to DAB has independent reason
to reject one or other of those claims. Part of the argument that this
is an \emph{independent} reason is that different sympathisers will
reject one rather than the other. To show this, I'll start with a short
story.
...
exemplar from Section~\ref{sec-lewis}. Both Auntie E and Auntie C think
...
that this action is desirable/good. Like Lewis does with \textbf{Binary
...
Before Peter acts, both Aunties have the same credal distribution,
...
\begin{longtable}[]{@{}rcc@{}}
\caption{The Aunties' credence that Peter will do A, and that it will be
...
haven't told you one thing extra - I haven't told you \sout{what
decision theory the two Aunties follow} {\bfseries\boldmath{}how they
generate values from credences}.

\sout{Auntie E is an evidential decision theorist.}
{\bfseries\boldmath{}Auntie E uses a method that is inspired by
evidential decision theory (EDT).} For her, the value of an arbitrary
descriptive proposition A is given by \textbf{Auntie E's Value}.
...
that she is most confident is good, conditional on Peter doing it.
{\bfseries\boldmath{}This is \emph{inspired} by EDT rather than just
being EDT because it is Auntie E's theory of desirability, not of
choiceworthiness, and a decision theory is a theory of choiceworthiness.
Indeed, notable causal decision theorist David Lewis
(\citeproc{ref-Lewis1981bn}{1981}) agrees with Auntie E that this is the
right theory of desirability. And for anyone with the Aunties' values
for worlds, this function is compulsory given \textbf{Additivity}. The
Aunties are moralists, so with \textbf{Binary Desirability} in place,
the worlds where Peter does the right thing, the worlds where A
\(\leftrightarrow\) Å is true, have value 1, and the rest value 0. Given
those world-values, \textbf{Additivity} entails \textbf{Auntie E's
Value}. So Auntie E will be our exemplar of a DAB theorist who accepts
\textbf{Additivity}.}

Auntie E hopes Peter does not do \emph{a}, since then she'll have
...
thing, which isn't as good. {\bfseries\boldmath{}Note that Auntie E's
values are demonstrably inconsistent with the conjunction of
\textbf{Equation} and \textbf{Additivity}. Her value function satisfies
\textbf{Additivity}; that is how it was constructed. But V(A) = Pr(Å
\textbar{} A) = 0.8, while Pr(Å) = 0.59, so \textbf{Equation} fails.}

\sout{Auntie C endorses a version of causal decision theory, in
particular something like the version supported by David Lewis (1981).
(Footnote: Here I'm following Collins, who notes that it is odd that
Lewis attributes to Auntie a form of evidential decision theory, which
Lewis himself does not endorse.) In particular, Auntie's values are
given by Auntie C's Value. In the formula, C\_x be the result of imaging
the credence function C on the proposition x is performed. Auntie C
believes changing the moral facts is a bigger change to the world than
changing any descriptive facts, so imaging always moves credences up or
down in Table 1, never left or right. (Footnote: Recall here that the
worlds are epistemic possibilities, not metaphysical ones, so it makes
sense to talk about merely changing the moral facts.)}

\begingroup\bfseries\boldmath

Auntie C uses a method that is inspired by causal decision theory (CDT),
and in particular by the version of CDT defended by David Lewis
(\citeproc{ref-Lewis1981bn}{1981}), in the spirit of Collins
(\citeproc{ref-Collins2015}{2015}). As with Auntie E, this is
inspiration rather than endorsement, because Auntie C is using it as a
theory of desirability, not of choiceworthiness. Lewis's equation (11)
is a theory of usefulness; it measures the thing a rational actor
maximises. Auntie C transposes it into a measure of the thing an
altruistic friend hopes is maximised. Whether Lewis really can allow
that these come apart is a question I'll return to in
Section~\ref{sec-auntie-c}. Her values are given by \textbf{Auntie C's
Value}, where Pr\textsubscript{A} is the result of \emph{imaging} the
credence function Pr on A.
...
V(A) = Pr\textsubscript{A}(Å)
\end{description}

\begingroup\bfseries\boldmath

Imaging moves the credence in each world to the nearest world where the
imaged proposition is true, so it requires a standard of nearness, and
here Auntie C has a distinctive view. She thinks that changing the moral
facts is a bigger change to the world than changing any descriptive
facts, so imaging always moves credences up or down in
Table~\ref{tbl-credence}, never left or right. Auntie C has her reasons
for imaging in this way. Morality is normally taken to be necessary, and
action is contingent, and a difference between worlds in necessary
matters is a bigger difference than a difference in contingent matters.
Relatedly, the literature on imaginative resistance suggests that we
find worlds with different moral truths harder to take seriously than
worlds that are impossible in other ways
(\citeproc{ref-Gendler2000}{Gendler 2000};
\citeproc{ref-Weatherson2004-WEAMFA}{Weatherson 2004}).

Given \textbf{Binary Desirability}, these commitments entail
\textbf{Equation}. Å is a purely moral proposition, and imaging on a
descriptive proposition never moves credence between worlds that differ
morally, so Pr\textsubscript{A}(Å) = Pr(Å). That is, V(A) = Pr(Å).
Nothing in that reasoning turned on which descriptive proposition A is.
So Auntie C will be our exemplar of a DAB theorist who accepts
\textbf{Equation}. The assumption of \textbf{Binary Desirability} is
still in play, so we haven't shown that she accepts \textbf{Equation} in
full generality. But that would be too much to ask of her.
\textbf{Equation} was formulated from the start as a claim about what
DAB comes to given \textbf{Binary Desirability}, and \textbf{Binary
Desirability}, as I said in Section~\ref{sec-lewis}, is an absurd
simplification. What Auntie C's values look like when worlds come in
more than two grades of desirability is a fair question, but not one
that Lewis's argument, which has \textbf{Binary Desirability} as a
premiss, entitles us to press.

\endgroup

\sout{This resembles equation (11) in `Causal Decision Theory'. Indeed,
...
We'll come back to this point in Section 4. For now, I just want to note
the similarities to Lewis's own theory.}

\sout{Using this formula, Auntie C hopes that Peter does \emph{a} iff
Pr(Å) \textgreater{} Pr(¬Å). Since Pr(Å) = 0.59, and Pr(¬Å) = 0.41, that
means she does hope that Peter does \emph{a}.}

\begingroup\bfseries\boldmath

So what does Auntie C hope Peter does? Imaging on A slides the credence
in each column of Table~\ref{tbl-credence} to the top row. That leaves
Pr\textsubscript{A}(Å) = 0.59, so V(A) = 0.59. Imaging on ¬A slides each
column to the bottom row, leaving Pr\textsubscript{¬A}(¬Å) = 0.41; and
since ¬Å is the proposition that not doing \emph{a} is good, V(¬A) =
0.41. So Auntie C hopes that Peter does \emph{a}. Her values are thus
demonstrably inconsistent with the conjunction of \textbf{Equation} and
\textbf{Additivity}, and we have already seen that \textbf{Equation}
holds, so it is \textbf{Additivity} that fails:
Σ\textsubscript{\emph{w}}V(\emph{w})Pr(\emph{w} \textbar{} A) = Pr(Å
\textbar{} A) = 0.8, while V(A) = 0.59.

\endgroup

The next two steps are to see why the Aunties reject Lewis's argument.

\section{Rejecting Equation}\label{sec-auntie-e}
...
Everyone agrees that beliefs play a role in instrumental desires. If one
...
view in general; \sout{I'm not an evidential decision theorist.} I'm
just defending the claim that this is a kind of desire as belief.
...
worrying once we remember what kinds of things worlds are in this
context. They are the elements of the contents of attitudes. That is, on
Lewis's view, they are centered worlds. Auntie E's view is really that
by necessity an agent desires that one of the centered worlds found
...
belief about \emph{desirability} unless it is matched by a desire. Lewis
...
\sout{So EDT offers a way out of Lewis's argument. That's not totally
surprising; it's not Lewis's view. Things are trickier with Auntie C.}
{\bfseries\boldmath{}So Auntie E has a way out of Lewis's argument. And
it is not an expensive way out. Her theory of desirability is one Lewis
himself accepts; the dispute between them is only over whether the
resulting position counts as desire as \emph{belief}, and Lewis's
reasons for saying it doesn't are not ones he can consistently give.
Things are trickier with Auntie C, because her way out requires denying
something Lewis explicitly asserts: that desirability is evidential.}

\section{Rejecting Additivity}\label{sec-auntie-c}

Auntie C says that the misstep in Lewis's argument is
\textbf{Additivity}. \sout{Following Collins, she says that this is
something that only an adherent of EDT should accept. She's mostly
mystified about why Lewis, famously an enemy of EDT, would have accepted
it in the first place.} {\bfseries\boldmath{}Like Collins, she rejects
Lewis's idea that desirability should follow different equations than
choiceworthiness, and like both Collins and Lewis, she thinks that the
right theory of choiceworthiness is a version of CDT not EDT.}

In the second DAB paper, Lewis\sout{has a response to this}
{\bfseries\boldmath{}explicitly notes that his argument relies on this
distinction}. Oddly, it's in a parenthetical paragraph. After describing
an action that's essentially two-boxing in Newcomb's problem, he writes

\begin{quote}
Should you take that action?---Yes \ldots{} Do you desire to perform
it?---No \ldots{} {[}O{]}ur topic here is not choiceworthiness but
desire \ldots{} {[}so{]} we adopt an `evidential' conception of expected
value \ldots{} Choiceworthiness is governed by a different `causal'
conception. (\citeproc{ref-Lewis1996}{Lewis 1996, 303})
\end{quote}

So Lewis thinks that \sout{Auntie C is confusing the two notions, and
that she only rejects \textbf{Additivity} because of this confusion}
{\bfseries\boldmath{}Auntie C is mistaken in inferring from CDT's
correctness as a theory of choiceworthiness to the correctness of
\textbf{Auntie C's Value} as a theory of desirability}. But there's
...
The first quote of Lewis's suggests that both Aunties should desire that
Peter choose Up, since that will be evidence that an outcome of value 3
will obtain. But it seems coherent to hope that he chooses Down. Several
of the arguments for two-boxing seem replicable here. It's weird to hope
that Peter chooses Down conditional on Left, and hope he chooses Down
conditional on Right, and unconditionally hope that he chooses Up.
{\bfseries\boldmath{}A benevolent and morally omniscient angel would
hope that Peter chooses Down, since whichever of Left and Right is true,
Down is the morally better choice. And even if Auntie C did not know why
the angel hopes this, knowing that the angel hopes it would seem like a
reason for her to hope it too.}
...
can't equate news-value with desirability. So \textbf{Additivity} has to
fail, since as Lewis agrees, \textbf{Additivity} entails that
desirability goes with news value.

\section{Conclusion}\label{sec-conclusion}

\sout{Any defender of DAB has to pick whether to take Auntie E's side or
Auntie C's side in the original question about Peter.}
{\bfseries\boldmath{}Any defender of DAB has to say what someone in the
Aunties' position should hope Peter does. Hoping he does not do \emph{a}
means giving up \textbf{Equation}; that is Auntie E's side. Hoping he
does \emph{a} means giving up \textbf{Additivity}; that is Auntie C's
side. Or at least it's the side of someone who shares her values,
whether or not they share her reasons for them. (I've given Auntie C the
most plausible view I know of that does end up hoping Peter does
\emph{a}, but that her reasons are optimal isn't important; what is
important is that she's got a viable view and to reject DAB Lewis owes
us an argument that she is mistaken.) And indifference means giving up
both.} Whichever side they pick, they will have an independent reason to
reject one of the premisses Lewis puts forward in his argument against
their view. So they have independent reason to reject Lewis's argument.

I've been stressing \emph{independent} here because it's not news that
any defender of DAB has to reject some premiss of Lewis's argument. That
follows from the fact that his argument is valid! For the response to be
more than table-thumping, the proponent of DAB has to say which premiss
they reject, and why it is reasonable to reject it. The point of the
examples involving Peter is to identify the premiss in question, which
...
\bibitem[\citeproctext]{ref-Weatherson2004-WEAMFA}
Weatherson, Brian. 2004. {``{Morality, Fiction, and Possibility}.''}
\emph{Philosophers' Imprint} 4 (3): 1--27.
