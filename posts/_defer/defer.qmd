...
Assume for now that the coin landed heads, so H is true. We'll work out the joint density function for A and B. Then we can work out the same density function conditional on \negH, and from those two facts work out the posterior probability of H. Call this value *h*. Conditional on A(p) = *a*, and B(p) = *b*, C's probability for p should be (1-h)*a* + *hb*. That's because conditional on A(p) = *a*, B(p) = *b* and H, C's probability for p should be *b*, while conditional on A(p) = *a*, B(p) = *b* and \negH, C's probability for p should be *a*. The short version of what follows is that since *h* is a function of *a* and *b* and is always in (0,1), it follows that C obeys constraint 4.
