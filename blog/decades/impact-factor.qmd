...
So why is Online First, and its counterparts with other publishers, so prevalent? Let's look at some data first. @tbl-spring lists some Springer philosophy journals, with the number of papers currently in the Online First section. If there's a 0 I couldn't find the section, and I think it doesn't exist, but I definitely could have make a mistake.^[Synthese is an interesting special case. They no longer have issues; like Imprint each article comes out as a self-standing piece, with a page number starting at 1. So every article is in final form as soon as it hits the website.]
